...
export default function LoginScreen() {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        const profRes = await withTimeout(
          (async () => await supabase.from('users').select('is_baker').eq('id', userId).maybeSingle())(),
          5000,
          'fetchIsBaker'
        );
  ...
  const handleLogin = async () => {
    ...
    // Başarılıysa _layout.tsx yönlendirir, ama baker'da setup kontrolü gerekebilir
    // _layout.tsx isBaker kontrolü yapıyor — yönlendirmeyi ona bırak
  };
  ...
  const handleForgotPassword = async () => {
    ...
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(trimmedEmail, {
      redirectTo: authRedirectUrl('pastacim-pro', true),
    });
    setIsResetLoading(false);
    if (resetError) {
  ...
  const handleSignup = async () => {
    ...
    const { error: authError, alreadyExisted, signedIn } = await signUp({
      ...
      redirectTo: authRedirectUrl('pastacim-pro'),
    });
    setIsLoading(false);
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      await navigateAfterAuth(s.user.id);
    } catch (e) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ─── Ayraç + Sosyal Giriş (email, login, signup modlarında) ── */}
