-- place_order: yeni sipariş açılınca yakındaki pastacılara server-side
-- bildirim + Expo push gönder. Eskiden bu client tarafında (customer app)
-- yapılıyordu; web siparişlerinde çalışmıyordu. Artık RPC içinde, tüm
-- platformlarda (native + web) çalışır. Bildirim hatası siparişi etkilemez.
  ...
  BEGIN
    IF p_latitude IS NOT NULL AND p_longitude IS NOT NULL THEN
      ...
      LOOP
        ...
        FOR v_tok IN
          SELECT DISTINCT t
          FROM unnest(array_remove(ARRAY[v_baker.baker_push_token, v_baker.push_token], NULL)) AS t
